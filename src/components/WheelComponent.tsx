...
const WheelComponent = ({
    ...
    const onTimerTick = () => {
        ...
        if (duration < upTime) {
            ...
            if (winningSegment) {
                if (currentSegment.toLowerCase() === winningSegment.toLocaleLowerCase() && frames > segments.length) {
                    progress = duration / upTime
                    angleDelta =
    ...
    return (
        <div className={`${isHidden ? 'hidden' : 'visible'} cursor-pointer `} id={wheelId.current}>
            <canvas
                id={canvasId.current}
                width={dimension}
                height={dimension}
